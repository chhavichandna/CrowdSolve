...
//     try {
//       if (editId) {
//         await axios.put(`https://crowdsolve-1-2pvf.onrender.com/api/questions/${editId}`, data, {
//           headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' },
//         });
//         setEditId(null);
//       } else {
//         await axios.post('https://crowdsolve-1-2pvf.onrender.com/api/questions', data, {
//           headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' },
//         });
//       }
...
//     try {
//       await axios.delete(`https://crowdsolve-1-2pvf.onrender.com/api/questions/${id}`, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       fetchQuestions();
//     } catch (err) {
...
const PDash = () => {
  ...
  return (
    <div className="p-6 pt-20 text-white min-h-screen bg-gradient-to-r from-gray-900 via-gray-800 to-gray-700">
      ...
      <form
        onSubmit={handleSubmit}
        className="mb-6 space-y-4 bg-gray-800 p-6 rounded-lg shadow-lg mt-6"
      >
        <input
          className="w-full p-3 bg-gray-700 rounded text-white"
          placeholder="Question Title"
        ...
        {form.image && (
          <img
            ...
            alt="Preview"
            className="w-40 h-40 object-cover rounded mt-3 border border-gray-600"
          />
        )}
      ...
      <div className="space-y-4">
        {questions.map((q) => (
          <div
            key={q._id}
            className="bg-gray-800 p-4 rounded shadow-md max-w-4xl mx-auto"
          >
            <h2 className="text-xl font-bold">{q.title}</h2>
            <p className="text-gray-300 mt-1">{q.description}</p>
            <p className="text-gray-400 text-sm mt-1">Location: {q.location}</p>

            {/* ✅ Cloudinary image will show automatically */}
            {q.image && (
              <img
                src={q.image}
                alt={q.title}
            ...
            <div className="mt-4 space-y-3">
              <h3 className="text-lg font-semibold text-white">Answers</h3>
              {q.answers.length === 0 && (
                <div className="text-gray-400 italic">No answers yet</div>
              )}
              {q.answers.map((ans) => {
                const key = `${q._id}-${ans._id}`;
                return (
                    ...
                    <div className="flex justify-between">
                      ...
                      <div className="flex flex-col items-center ml-4">
                        <button
                          ...
                          className="p-2 rounded-full transform hover:scale-105 transition"
                        >
                          <FaHeart
                            className={`w-6 h-6 ${
                              ans.likes && ans.likes.length > 0
                                ? 'text-red-500'
                                : 'text-gray-400'
                            }`}
                          />
