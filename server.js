...
// // ====== Middleware ======
// app.use(cors({
//   origin: "*", // Allow all origins for Render; change if needed
//   methods: ["GET", "POST", "PUT", "DELETE", "OPTIONS"],
//   allowedHeaders: ["Content-Type", "Authorization"],
// }));
// app.use(express.json());
...
//     const formatted = questions.map(q => ({
//       ...q._doc,
//       image: q.image ? `${req.protocol}://${req.get("host")}${q.image}` : null,
//     }));
//     res.json(formatted);
//   } catch (err) {
//     res.status(500).json({ msg: err.message });
...
// Post Question (Cloudinary upload)
app.post('/api/questions', authMiddleware, upload.single('image'), async (req, res) => {
  const { title, description, location } = req.body;
  try {
...
app.get('/api/questions', authMiddleware, async (req, res) => {
  try {
    ...
    const formatted = questions.map(q => ({
      ...q._doc,
      image: q.image || null, // Cloudinary gives full URL
    }));
    res.json(formatted);
  } catch (err) {
    res.status(500).json({ msg: err.message });
